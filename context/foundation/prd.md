...
- End-to-end MVP flow works: register/login → ask question → cited answer with source list → saved to private history → re-view from history without new answer generation.
- ≥90% of generated answers include at least 2 cited sources.
...
- **Then** they see the same stored answer and sources with no new answer generation
...
- FR-005: User can view a source list (title, URL, snippet) for each answer. Priority: must-have
  > Socrates: No counter-argument; stands as written.
- FR-009: System limits how frequently a user can submit questions. Priority: must-have
...
- FR-007: User can browse their past questions and answers. Priority: must-have
  > Socrates: No counter-argument; stands as written.
- FR-008: User can re-open a past answer from stored history without triggering a new answer generation. Priority: must-have
  > Socrates: No counter-argument; stands as written.
...
The user supplies a natural-language question. The system retrieves current web-grounded content and produces a single answer text where claims reference numbered sources. The user sees inline citation markers in the answer body and a parallel source list with title, URL, and snippet for each reference — they can open any source to judge credibility themselves. After generation, the question, answer, and sources are persisted to that user's private history; revisiting a history entry replays the stored response without a new answer generation.
...
- **Custom web crawling / indexing** — no owned crawl pipeline; web grounding relies on an external provider.
...
- **Incremental answer display** — full answer displayed when ready, not token-by-token.

## Open Questions

No open questions recorded from shaping. Quality cross-check: all soft-gate elements present.
